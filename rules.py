...
def verb_pron_break(word):
    ...
    elif len(word) > 3 and word.endswith("ود") and "بود" not in word and "نمو" not in word:
        return word[:-2] + "ه"
    elif len(word) > 3 and word.endswith("د") and not word.endswith("اد") and not word.endswith(
            "ید") and not word.endswith("ند") and not word.endswith("کرد"):
        return word[:-1] + "ه"
...
def less_common_deterministic_break(word, tag):
    ...
    if word == "انبر":
        return "امبر"
    if word == "بالای" and random.random() < 0.3:
        return "رو"
    if word == "بالای" and random.random() < 0.3:
        return "روی"
    if word == "پنبه":
...
def deterministic_break(word, tag):
    ...
    if word == "چهل":
        return "چل"
    if word == "چه" and tag == "DET" and random.random() < 0.5:
        return "چی"
    if word == "چیست":
        return "چیه"
    if word == "حرام":
